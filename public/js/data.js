...
const INITIAL_POSTS = [
  {
    ...
    content_body: { en: `## ⚡ The Unveiling of GPT-4o: A Revolution in Real-Time Communication\nOpenAI has officially launched **'GPT-4o (o for omni)'**, a new flagship artificial intelligence model that can see, hear, and speak in real-time, just like a human. It processes text, audio, and images natively, enabling seamless, latency-free conversations.\n\n## 🗣️ The Movie 'Her' Becomes Reality\nCEO Sam Altman praised it, saying, "The AI from the movies has become reality." While previous models experienced 2-3 seconds of latency due to converting speech to text, GPT-4o responds in an average of 320 milliseconds (0.32 seconds), similar to human conversation speed. It can even explain the solution to a math equation in real-time when pointed at with a smartphone camera.\n\n## 🚀 A Global AI Earthquake Expected\nThe most surprising aspect is that this revolutionary model is available **not only to paid subscribers but also to free users**. OpenAI plans to roll it out gradually to users worldwide over the coming weeks, which is expected to put immense pressure on competitors.`, ko: `## ⚡ 베일 벗은 GPT-4o, 실시간 소통의 혁명\n오픈AI가 5월 13일(현지시각), 사람처럼 실시간으로 보고 듣고 말할 수 있는 새로운 플래그십 인공지능 모델 **'GPT-4o(포오)'**를 전격 공개했습니다. 텍스트, 오디오, 이미지를 동시에 처리할 수 있어 지연 시간 없는 자연스러운 대화가 가능한 것이 특징입니다.\n\n## 🗣️ 영화 '그녀(Her)'가 현실로\n샘 알트만 CEO는 "영화 속 AI가 현실이 되었다"며 극찬했습니다. 기존 모델은 음성을 텍스트로 변환한 뒤 처리하느라 2~3초의 지연이 있었으나, GPT-4o는 평균 320밀리초(0.32초) 만에 응답해 사람 간 대화 속도와 비슷합니다. 또한 스마트폰 카메라로 방정식을 비추면 실시간으로 풀이 과정을 설명해주기도 합니다.\n\n## 🚀 전 세계 AI 지각변동 예고\n가장 놀라운 점은 이 혁신적인 모델이 유료 구독자뿐만 아니라 **무료 사용자에게도 개방**된다는 것입니다. 오픈AI는 몇 주에 걸쳐 전 세계 사용자들에게 순차적으로 배포할 예정이며, 이는 경쟁사들에게 엄청난 압박으로 작용할 것으로 보입니다.` },
  ...
  },
  {
    ...
    content_body: { en: `## 📰 A Mid-Weight Model Sweeping the Benchmarks\n**Anthropic** has unexpectedly released **'Claude 3.5 Sonnet'**, a mid-weight model that outperforms their previous flagship model, setting new benchmark standards across the industry.\n\n## 📖 'Artifacts': An Innovative Real-Time UI\nThe newly released 3.5 Sonnet dominated OpenAI's GPT-4o in key performance metrics, including graduate-level reasoning. It also operates more than twice as fast as the previous generation's top model (Opus). A key highlight of this announcement is the **'Artifacts'** feature, which opens a dedicated panel on the right side of the chat window to render React code or graphics generated by the AI in real-time.\n\n## 📌 Service Availability\nThis model is currently available for anyone to use for free on the Claude.ai website and is also officially supported on the mobile app.`, ko: `## 📰 벤치마크를 휩쓴 중간 체급 모델\n**Anthropic(앤스로픽)**이 기존 자사의 최상위 모델을 뛰어넘는 중간 체급 모델 **'Claude 3.5 소넷(Claude 3.5 Sonnet)'**을 기습 출시하여 업계의 벤치마크 기준을 갈아치웠습니다.\n\n## 📖 혁신적인 실시간 UI '아티팩트'\n새롭게 출시된 3.5 Sonnet은 경쟁사인 OpenAI의 GPT-4o를 대학원 수준의 추론 등 주요 성능 평가 지표에서 압도했습니다. 작동 속도 또한 이전 세대 최상위 모델(Opus)보다 2배 이상 빨라졌습니다. 특히 이번 발표의 핵심은 대화창 우측에 전용 패널을 띄워 AI가 작성한 리액트(React) 코드나 그래픽을 실시간으로 렌더링해 주는 **'Artifacts(아티팩트)'** 기능입니다.\n\n## 📌 서비스 이용 안내\n이 모델은 현재 Claude.ai 웹사이트에서 누구나 무료로 사용해 볼 수 있으며, 모바일 앱에서도 정식 지원됩니다.` },
  ...
  },
  {
    ...
    content_body: { en: `## 📰 The Magic of 1 Million Tokens\nGoogle has unveiled its next-generation AI model, **'Gemini 1.5 Pro'**, capable of analyzing up to 1 hour of video or 30,000 lines of code in a single prompt. This boasts the most massive context window of any commercial model currently available.\n\n## 📖 Analyzing Entire Codebases and Videos at Once\nThe core of this update is the introduction of a 'Mixture-of-Experts (MoE)' architecture, which maximizes computational efficiency. As a result, it is now possible to upload hundreds of pages of PDF documents or videos over an hour long to be summarized and searched entirely at once.\n\n## 📌 Developer Preview and Future Plans\nGoogle is initially offering a preview version of this model to select developers and enterprise customers, with plans to gradually expand the rollout to general users.`, ko: `## 📰 100만 토큰 처리의 마법\n구글이 단일 프롬프트로 최대 1시간 분량의 동영상이나 3만 줄의 코드를 한 번에 분석할 수 있는 차세대 AI 모델 **'Gemini 1.5 Pro'**를 전격 공개했습니다. 이는 기존 상용 모델 중 가장 거대한 문맥 파악 범위를 자랑합니다.\n\n## 📖 영상부터 코드베이스 통째 분석까지\n이번 업데이트의 핵심은 'Mixture-of-Experts (MoE)' 아키텍처를 도입하여 연산 효율을 극대화한 것입니다. 덕분에 수백 페이지 분량의 PDF 문서나 1시간이 넘는 긴 영상을 통째로 업로드하여 요약하고 검색하는 것이 가능해졌습니다.\n\n## 📌 개발자 프리뷰 및 향후 계획\n구글은 일부 개발자와 기업 고객을 대상으로 이 모델의 프리뷰 버전을 우선 제공하며, 점진적으로 일반 사용자들에게도 배포를 확대할 예정입니다.` },
  ...
  },
  {
    ...
    content_body: { en: `## 📰 Imagination Becomes Reality: Text to Video\nOpenAI has sent shockwaves through the global video industry by unveiling **'Sora'**, an AI model capable of generating stunning, photorealistic videos up to 1 minute long from just a single line of prompt.\n\n## 📖 A World Simulator That Understands Physics\nSora goes beyond merely stitching pixels together; it generates video based on an 'understanding of the physical world'—how subjects move and interact in a 3D space. It flawlessly renders hyper-realistic scenes, such as a woman walking down a snowy Tokyo street or a mammoth with vividly lifelike fur texture.\n\n## 📌 Ethical Challenges\nCurrently, Sora is only available in a restricted capacity to select visual artists and filmmakers for safety and red-teaming evaluations.`, ko: `## 📰 상상이 현실로, 텍스트가 영상으로\n오픈AI(OpenAI)가 프롬프트 한 줄만으로 최대 1분 길이의 압도적인 실사급 동영상을 생성할 수 있는 AI 모델 **'Sora(소라)'**를 공개하여 전 세계 영상 산업에 큰 충격을 던졌습니다.\n\n## 📖 물리 법칙을 이해하는 세계 시뮬레이터\n소라는 단순히 픽셀을 짜깁기하는 수준을 넘어, 피사체가 3차원 공간에서 어떻게 움직이고 상호작용하는지에 대한 '물리적 세계의 이해'를 바탕으로 영상을 생성합니다. 눈보라 치는 도쿄 거리를 걷는 여성, 털의 질감이 생생하게 살아있는 매머드 등 극사실적인 영상을 완벽하게 소화해냅니다.\n\n## 📌 윤리적 과제\n현재 소라는 안전성 평가를 위해 일부 시각 예술가와 영화 제작자에게만 제한적으로 권한이 부여된 상태입니다.` },
    program_l2: "Sora",
    category_l1: "video",
    company: "OpenAI",
  ...
  },
  {
    ...
    content_body: { en: `## 📰 AI Encroaches on the Human Developer's Domain\nCognition, an American AI startup, has announced **'Devin'**, the world's first fully autonomous AI software engineer that can independently plan, research, code, debug, and deploy software from a single prompt.\n\n## 📖 A Flawless Colleague Working 24/7\nDevin comes equipped with its own virtual sandbox environment, including a shell, code editor, and browser. It googles like a human, writes code, and if an error occurs, it autonomously finds the cause and fixes it. It has already proven its remarkable autonomy by flawlessly completing actual client requests on platforms like Upwork.\n\n## 📌 A Paradigm Shift in the Software Ecosystem\nDevin is currently available in early access to select companies. It is poised to be a key catalyst in drastically lowering the barrier to entry for software development and shifting the role of human developers from 'coders' to 'reviewers'.`, ko: `## 📰 AI, 인간 개발자의 자리를 넘보다\n미국의 AI 스타트업 코그니션(Cognition)이 지시사항만 내리면 기획부터 코딩, 디버깅, 배포까지 스스로 수행하는 완전 자율형 AI 엔지니어 **'Devin(데빈)'**을 전격 발표했습니다.\n\n## 📖 24시간 일하는 완벽한 동료\n데빈은 터미널, 코드 편집기, 브라우저가 포함된 자체 가상 환경을 갖추고 있어 인간처럼 구글링을 하고, 코드를 작성한 뒤 에러가 나면 스스로 원인을 찾아 수정합니다. 실제로 실제 고객의 의뢰를 완벽하게 수행해내며 놀라운 자율성을 입증했습니다.\n\n## 📌 소프트웨어 생태계의 전환\n현재 데빈은 초기 액세스 형태로 일부 기업에 제공되고 있으며, 개발자의 역할을 '코더'에서 '리뷰어'로 변화시킬 핵심 기폭제가 될 전망입니다.` },
    program_l2: "Devin",
    category_l1: "code",
    company: "Cognition",
